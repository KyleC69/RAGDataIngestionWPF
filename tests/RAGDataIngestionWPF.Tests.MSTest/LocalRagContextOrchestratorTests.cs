...
// File:         LocalRagContextOrchestratorTests.cs
...
public class LocalRagContextOrchestratorTests
{
    ...
    public async Task BuildContextMessagesFusesExpandedQueriesByRank()
    {
        ...
        retrieval.Setup(x => x.SearchAsync(new RagSearchQuery("latest schema question"), It.IsAny<CancellationToken>())).ReturnsAsync(new List<RagSearchResult> { new(2, "Second", "Summary 2", new[] { "schema" }, 0.95), new(1, "First", "Summary 1", new[] { "question" }, 0.70) });
        retrieval.Setup(x => x.SearchAsync(new RagSearchQuery("schema question"), It.IsAny<CancellationToken>())).ReturnsAsync(new List<RagSearchResult> { new(1, "First", "Summary 1", new[] { "schema" }, 0.60), new(3, "Third", "Summary 3", new[] { "schema" }, 0.50) });

        LocalRagContextOrchestrator orchestrator = new(retrieval.Object, CreateQueryExpander(new RagSearchQuery("latest schema question"), new RagSearchQuery("schema question")).Object, new ContextCitationFormatter(), CreateSettings().Object);

        var contextMessages = await orchestrator.BuildContextMessagesAsync([new(ChatRole.User, "latest schema question")], CancellationToken.None);

        Assert.AreEqual(1, contextMessages.Count);
        var firstIndex = contextMessages[0].Text.IndexOf("First", StringComparison.Ordinal);
        var secondIndex = contextMessages[0].Text.IndexOf("Second", StringComparison.Ordinal);
    ...
    public async Task BuildContextMessagesRespectsCharacterBudget()
    {
        ...
        Mock<IRagRetrievalService> retrieval = new();
        retrieval.Setup(x => x.SearchAsync(new RagSearchQuery("latest"), It.IsAny<CancellationToken>())).ReturnsAsync(new List<RagSearchResult> { new(1, "First", largeSummary, new[] { "a" }, 0.9), new(2, "Second", largeSummary, new[] { "b" }, 0.8) });
        ...
        var contextMessages = await orchestrator.BuildContextMessagesAsync([new(ChatRole.User, "latest")], CancellationToken.None);
    ...
    {
        Mock<IRagRetrievalService> retrieval = new();
        retrieval.Setup(x => x.SearchAsync(new RagSearchQuery("latest"), It.IsAny<CancellationToken>())).ReturnsAsync(new List<RagSearchResult> { new(2, "Second", "Summary 2", new[] { "b" }, 0.6), new(1, "First", "Summary 1", new[] { "a" }, 0.9), new(1, "First Duplicate", "Duplicate summary", new[] { "a" }, 0.1) });

        var expander = CreateQueryExpander(new RagSearchQuery("latest"));
        ...
        var contextMessages = await orchestrator.BuildContextMessagesAsync(requestMessages, CancellationToken.None);
    ...
    private static Mock<IRagQueryExpander> CreateQueryExpander(params RagSearchQuery[] queries)
    {
        ...
        expander.Setup(x => x.Expand(It.IsAny<IReadOnlyList<ChatMessage>>())).Returns(queries);
        return expander;
    }
