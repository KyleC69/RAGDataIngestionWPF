...
// File:         ConversationProgressLogServiceTests.cs
...
public class ConversationProgressLogServiceTests
{
    ...
    public async Task AbandonPlanAsyncMarksPlanAbandonedAndStoresReason()
    {
        ...
        store.Setup(x => x.GetAsync("conversation-1", planId, It.IsAny<CancellationToken>()))
        ...
        store.Setup(x => x.SaveAsync(It.IsAny<ConversationProgressLog>(), It.IsAny<CancellationToken>())).Callback<ConversationProgressLog, CancellationToken>((plan, _) => saved = plan).Returns(ValueTask.CompletedTask);
    ...
    public async Task RecordArtifactAsyncUpsertsArtifact()
    {
        Guid planId = Guid.NewGuid();
        Mock<IConversationProgressLogStore> store = new();
        store.Setup(x => x.GetAsync("conversation-1", planId, It.IsAny<CancellationToken>())).ReturnsAsync(new ConversationProgressLog { PlanId = planId, ConversationId = "conversation-1", PlanName = "rewrite", Artifacts = [] });
        store.Setup(x => x.SaveAsync(It.IsAny<ConversationProgressLog>(), It.IsAny<CancellationToken>())).Returns(ValueTask.CompletedTask);
        IConversationProgressLogService service = new ConversationProgressLogService(store.Object);
