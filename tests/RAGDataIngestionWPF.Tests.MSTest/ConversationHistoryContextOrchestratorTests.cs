...
// File:         ConversationHistoryContextOrchestratorTests.cs
...
public class ConversationHistoryContextOrchestratorTests
{
    ...
    public async Task BuildContextMessagesRespectsMetaBudget()
    {
        ...
        loader.Setup(x => x.LoadConversationHistoryAsync("conversation-42", It.IsAny<CancellationToken>()))
                .ReturnsAsync([
        ...
        var messages = await orchestrator.BuildContextMessagesAsync("conversation-42", [new(ChatRole.User, "startup schema validation")], CancellationToken.None);

        Assert.AreEqual(1, messages.Count);
        Assert.IsTrue(messages[0].Text.Contains("startup schema validation second", StringComparison.Ordinal) || messages[0].Text.Contains("startup schema validation ", StringComparison.Ordinal));
        var blockCount = messages[0].Text.Split("[", StringSplitOptions.RemoveEmptyEntries).Length - 1;
    ...
    public async Task BuildContextMessagesReturnsRankedRelevantHistory()
    {
        ...
        loader.Setup(x => x.LoadConversationHistoryAsync("conversation-42", It.IsAny<CancellationToken>()))
                .ReturnsAsync([
                        new ChatMessage(ChatRole.Assistant, "General setup note") { CreatedAt = new DateTimeOffset(2026, 3, 10, 8, 0, 0, TimeSpan.Zero) },
                        new ChatMessage(ChatRole.Assistant, "Schema validation should run during startup initialization.") { CreatedAt = new DateTimeOffset(2026, 3, 11, 8, 0, 0, TimeSpan.Zero) },
                        new ChatMessage(ChatRole.User, "We should review schema validation for startup.") { CreatedAt = new DateTimeOffset(2026, 3, 12, 8, 0, 0, TimeSpan.Zero) }
                ]);

        ConversationHistoryContextOrchestrator orchestrator = new(loader.Object, new ContextCitationFormatter(), CreateSettings().Object);

        var messages = await orchestrator.BuildContextMessagesAsync("conversation-42", [new(ChatRole.User, "How does schema validation work at startup?")], CancellationToken.None);
    ...
    public async Task BuildContextMessagesSkipsHistoryThatDuplicatesRequestText()
    {
        ...
        var messages = await orchestrator.BuildContextMessagesAsync("conversation-42", [new(ChatRole.User, "Repeat this exact request")], CancellationToken.None);
