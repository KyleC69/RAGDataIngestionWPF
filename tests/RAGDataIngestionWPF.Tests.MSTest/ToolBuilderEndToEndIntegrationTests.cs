...
public class ToolBuilderEndToEndIntegrationTests
{
    ...
    public async Task GetAiToolsInvokesCuratedReadOnlyToolsAsync()
    {
        ...
        object systemInfoTool = FindToolByName(tools, "GetInfo");
        object performanceTool = FindToolByName(tools, "ReadSnapshot");

        object? systemInfoResult = await InvokeToolAsync(systemInfoTool, null).ConfigureAwait(false);
        object? performanceResult = await InvokeToolAsync(performanceTool, new Dictionary<string, object?>
        {
            ["sampleDelayMilliseconds"] = 250
    ...
    private static void AssertToolResultSucceeded(object? result, string failureMessage)
    {
        ...
        string? error = errorProperty.GetValue(result)?.ToString();
    ...
    private static void AssertJsonToolResultSucceeded(JsonElement result, string failureMessage)
    {
        ...
        string? error = TryGetProperty(result, "Error") is JsonElement errorElement && errorElement.ValueKind != JsonValueKind.Null
    ...
    {
        JsonElement? property = TryGetProperty(element, propertyName);
    ...
    private static object FindToolByName(IList tools, string name)
    {
        ...
        foreach (object tool in tools)
        {
            ...
            string? toolName = nameProperty?.GetValue(tool)?.ToString();
    ...
    private static async Task<object?> InvokeToolAsync(object tool, IDictionary<string, object?>? arguments)
    {
        ...
        ParameterInfo[] parameters = invokeAsync.GetParameters();
        object?[] parameterValues = parameters.Select(parameter => CreateArgumentValue(parameter, arguments)).ToArray();

        object? invocation = invokeAsync.Invoke(tool, parameterValues);
    ...
    private static object CreateArgumentsObject(Type argumentType, IDictionary<string, object?> suppliedArguments)
    {
        ...
        foreach (ConstructorInfo constructor in argumentType.GetConstructors(BindingFlags.Public | BindingFlags.Instance))
        {
            ...
            foreach (object variant in dictionaryVariants)
            {
        ...
        object argsInstance = Activator.CreateInstance(argumentType)!;
        MethodInfo? addMethod = argumentType.GetMethod("Add", BindingFlags.Public | BindingFlags.Instance);
        if (addMethod != null)
        {
            foreach ((string key, object? value) in suppliedArguments)
            {
                addMethod.Invoke(argsInstance, [key, value]);
            }
        }
