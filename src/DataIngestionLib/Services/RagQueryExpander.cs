...
public sealed class RagQueryExpander : IRagQueryExpander
{
    ...
    public IReadOnlyList<RagSearchQuery> Expand(IReadOnlyList<ChatMessage> requestMessages)
    {
        ...
        string latestQuery = requestMessages
        ...
        string keywordQuery = string.Join(" ", Tokenize(latestQuery).Take(8));
        if (!string.IsNullOrWhiteSpace(keywordQuery)
            && !string.Equals(keywordQuery, latestQuery, StringComparison.OrdinalIgnoreCase))
        {
